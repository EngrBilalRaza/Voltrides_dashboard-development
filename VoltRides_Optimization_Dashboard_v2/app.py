...
def render_sidebar(data: dict) -> dict:
    ...
    with st.sidebar.expander("3. Cost drivers", expanded=False):
        scrap_rate = st.slider("Scrap / rework rate", 0.0, 0.15, BASE_SCRAP_RATE, 0.005)
        energy_escalation = st.slider("Energy tariff escalation", -0.10, 0.50, 0.0, 0.01)
    ...
    with st.sidebar.expander("5. Station process times", expanded=False):
        ...
        for _, row in station_df.iterrows():
            station = row["station"]
            st.markdown(f"**{station} — {row.get('name', '')}**")
            c1, c2 = st.columns(2)
            station_times[station] = c1.number_input(
                f"{station} time",
                min_value=1.0,
                max_value=480.0,
            ...
            station_workers[station] = c2.slider(
                f"{station} workers",
                minimum,
