...
def assembly_sankey(sub_assemblies: pd.DataFrame):
    labels, index, sources, targets, values = [], {}, [], [], []
    def idx(label: str):
        if label not in index:
            index[label] = len(labels)
            labels.append(label)
        return index[label]
    plant = idx("Sub-Assembly Feed")
    for _, row in sub_assemblies.iterrows():
        sa_label = f"{row['station']} {short_label(row.get('description', ''), 18)}"
        ma_label = str(row.get("feeds_into", "MA"))
        sources.append(plant); targets.append(idx(sa_label)); values.append(max(1, float(row.get("workers", 1))))
        sources.append(idx(sa_label)); targets.append(idx(ma_label)); values.append(max(1, float(row.get("workers", 1))))
    fig = go.Figure(data=[go.Sankey(node=dict(label=labels, pad=20, thickness=18, color=COLORS["soft_blue"]), link=dict(source=sources, target=targets, value=values))])
    return apply_chart_layout(fig, height=540, title="Sub-assembly feeder flow into main assembly")
