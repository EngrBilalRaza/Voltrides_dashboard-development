...
def assembly_sankey(sub_assemblies: pd.DataFrame):
    """High-contrast Sankey chart for SA → MA feeder relationships."""
    ...
    def hex_to_rgba(hex_color: str, alpha: float = 0.32) -> str:
    ...
    plant_idx = idx("SA Feeder<br>Network", "#0F172A")
    ...
    for _, row in df.iterrows():
        ...
        description = short_label(str(row.get("description", "")), 24)
        ...
        sa_label = f"{station}<br>{description}"
        ma_label = f"{ma_station}<br>Main Assembly"

        sa_idx = idx(sa_label, "#FFFFFF")
        ma_idx = idx(ma_label, ma_color)

        sources.append(plant_idx)
        targets.append(sa_idx)
        values.append(worker_value)
        link_colors.append("rgba(15, 23, 42, 0.18)")
    ...
    fig.update_layout(
        font=dict(size=13, color="#0F172A", family="Inter, Arial, sans-serif"),
        margin=dict(l=18, r=18, t=72, b=24),
    )
    return apply_chart_layout(fig, height=570, title="Sub-assembly feeder flow into main assembly")
